...
select team,sum(silver_medals) as total_silver_medals,max(case when rn=1 then year end) as year_of_max_silver from cte
group by team;
...
where medal='Silver'
group by a.team,ae.year)
, ranked_silver_cnt as (
select team,year,silver_medals	
,sum(silver_medals) over(partition by team)  as total_silver_medals
,row_number() over(partition by team order by silver_medals desc, year) as rn
from cte)
select team,total_silver_medals,year as year_of_max_silver from ranked_silver_cnt
where rn=1
...
select top 1 a.name,
count(ae.medal) as no_of_gold_medals
from athletes a
inner join athlete_events ae
on a.id = ae.athlete_id
and ae.medal = 'Gold'
left join athlete_events s
on a.id = s.athlete_id
and s.medal in ('Silver','Bronze')
...
and ae2.medal in ('Silver','Bronze')
)
group by a.name
order by no_of_gold_medals desc
